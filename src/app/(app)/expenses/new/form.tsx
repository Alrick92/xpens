...
export function NewExpenseForm({ categories, projects }: FormProps) {
  ...
  const [pendingParsedData, setPendingParsedData] = useState<ParsedData | null>(null);
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      if (parseRes.ok && parseData.success) {
        const d = parseData.data as ParsedData;
        setPendingParsedData(d);
        setPreviewOpen(true);
      } else {
  ...
  function applyParsedData(d: ParsedData) {
    setParsedData(d);
    ...
    toast.success(
      `Receipt data applied (${Math.round(d.confidence * 100)}% confidence)`
    );
  }
  ...
  return (
    <>
      ...
      {pendingParsedData && uploadedFile && (
        <ReceiptPreviewDialog
          key={`${uploadedFile.url}-${pendingParsedData.confidence}`}
          open={previewOpen}
          onOpenChange={setPreviewOpen}
          fileUrl={uploadedFile.url}
          fileName={uploadedFile.filename}
          fileType={uploadedFile.originalType}
